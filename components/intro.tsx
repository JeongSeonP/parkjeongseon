  ...
  return (
    ...
    >
      <div className="w-full relative mt-3 mb-4 sm:mb-10">
        <motion.div
          className="mx-auto bg-amber-50 dark:bg-cyan-950 blur-xl w-full h-32 sm:w-1/2  rounded-full "
          initial={{ scale: 1 }}
          animate={{ scale: [1.1, 1] }}
        />
        <motion.div
          className="absolute inset-0  flex flex-col items-center justify-center leading-9 font-semibold text-xl text-slate-900 dark:text-gray-300"
          animate={{ scale: [1.1, 1] }}
          transition={{ type: "spring", damping: 10, stiffness: 100 }}
        >
          <p>안녕하세요. </p>
          <div className="sm:flex-row flex-col">
            <p className="text-sky-700" id="animate-puls-text">
              도전을 두려워 하지 않는
            </p>
            <p className="inline-block">주니어 Frontend 개발자</p>
            <p className="inline-block">박정선 입니다.</p>
          </div>
        </motion.div>
      </div>
      <div className="flex flex-col sm:flex-row items-center justify-center gap-10 mb-6">
        ...
        <div className="flex flex-col items-center gap-2 text-sm font-medium dark:text-slate-800">
          {profileDatas.map((data) => (
            <button
              key={data.name}
              className="bg-white dark:bg-slate-300 border border-indigo-300 hover:bg-indigo-50/60 dark:hover:bg-indigo-200 rounded-full shadow-md"
              type="button"
            >
              <a
